...
function OpenAIProvider({ children}: PropsWithChildren) {
    ...
    const [response, setResponse] = useState<IOpenAIResponse | undefined>(undefined);

    const sendMessage = useCallback(
      (body: IOpenAIRequest) => {
        return postOpenAI(`https://api.openai.com/v1/chat/completions`, body)
          .then((res: IOpenAIResponse) => {
            setResponse(res);
            return res;
          })
          .catch((error) => {
