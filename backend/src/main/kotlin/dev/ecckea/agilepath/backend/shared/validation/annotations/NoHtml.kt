package dev.ecckea.agilepath.backend.shared.validation.annotations

import dev.ecckea.agilepath.backend.shared.validation.validators.NoHtmlValidator
import jakarta.validation.Constraint
import jakarta.validation.Payload
import kotlin.reflect.KClass

/**
 * Annotation to validate that a field does not contain HTML content.
 * This annotation is used in conjunction with the `NoHtmlValidator` class.
 *
 * @property message The error message to be returned if validation fails. Defaults to "must not contain HTML".
 * @property groups Allows specification of validation groups. Defaults to an empty array.
 * @property payload Can be used by clients to assign custom payload objects to a constraint. Defaults to an empty array.
 */
@Target(AnnotationTarget.FIELD)
@Retention(AnnotationRetention.RUNTIME)
@Constraint(validatedBy = [NoHtmlValidator::class])
annotation class NoHtml(
    val message: String = "must not contain HTML",
    val groups: Array<KClass<*>> = [],
    val payload: Array<KClass<out Payload>> = []
)